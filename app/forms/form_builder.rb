...
class FormBuilder < ActionView::Helpers::FormBuilder
  ...
  FIELDTYPES.each do |method_name|
    define_method(method_name) do |name, options = {}|
      ...
      content_tag :div, class: class_definition(options.delete(:outer_class)) do
    ...
    content_tag :div, class: "row" do
      content_tag :div, class: "file-field input-field col s12" do
        content_tag(:div, class: "btn") do
          content_tag(:span) { options.delete(:label) } + super(method, options)
        end + content_tag(:div, class: "file-path-wrapper") do
  ...
  def class_definition(outer_class)
    if outer_class
      "row " + outer_class
    else
      "row"
